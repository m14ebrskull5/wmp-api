...
import com.wmp.api.common.constants.ApiDocTypeEnum;
import com.wmp.api.common.constants.SwaggerConstants;
...
/**
 * @ClassName: SwaggerApiDocUtils
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author chenaonan
 * @date 2018年8月8日 上午11:49:50
 */
public class SwaggerApiDocUtils {
	 ...
	 * 
	 * @param apiDocTypeEnum
	 *            文档类型枚举
	 * @return
	 * @throws MalformedURLException
	 * @throws IOException
	 */
	public static ResponseEntity<InputStreamResource> downloadApiDoc(ApiDocTypeEnum apiDocTypeEnum)
	...
	public static ResponseEntity<InputStreamResource> downloadApiDocJson() throws Exception {
		...
		return ResponseEntity.ok().headers(headers).contentType(MediaType.parseMediaType("application/octet-stream"))
				.body(new InputStreamResource(
						IOUtils.toInputStream(JacksonUtils.obj2jsonIgnoreNullPretty(swagger), "UTF-8")));
	...
	private static void generate(String swaggerURL, MarkupLanguage markupLanguage, String toFile)
			throws MalformedURLException {
		Swagger2MarkupConfig config = new Swagger2MarkupConfigBuilder().withMarkupLanguage(markupLanguage).build();

		Swagger2MarkupConverter
				// 例:http://localhost:8080/v2/api-docs
				.from(new URL(swaggerURL)).withConfig(config).build()
	...
	private static String generateHtml() throws IOException {
		...
		OptionsBuilder optionsBuilder = OptionsBuilder.options().backend("html5")// html
				.docType("book").eruby("").inPlace(true).safe(SafeMode.UNSAFE)
				.toFile(new File(SwaggerConstants.API_DOC_HTML_FILE)).attributes(attributes);
