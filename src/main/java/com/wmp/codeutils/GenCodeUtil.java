...
public class GenCodeUtil {
	 ...
	 * @return
	 */
	private static String genJavaBeanFromTableStructure(String author, TableModel table, String beanName,
			String packagePath) {
		...
		try {
			...
			// get set
			for (ColumnModel columnModel : columnModelList) {
				sb.append("\tpublic " + columnModel.getColumnClassName() + " get"
						+ toFirstCharUpCase((String) columnModel.getFieldName()) + "() {\r\n" + "\t\treturn "
						+ columnModel.getFieldName() + ";\r\n" + "\t}\r\n" + "\r\n" + "\tpublic void set"
						+ toFirstCharUpCase((String) columnModel.getFieldName()) + "("
						+ columnModel.getColumnClassName() + " " + columnModel.getFieldName() + ") {\r\n" + "\t\tthis."
	 ...
	 * @return
	 */
	private static String genQueryModelFromTableStructure(String author, TableModel table, String beanName,
			String extendsBasePackage, String packagePath) {
		StringBuffer sb = new StringBuffer();
		if (StringUtils.isNotEmpty(packagePath)) {
			sb.append("package " + packagePath + ";\n\n");
		}
		...
		try {
			...
			for (ColumnModel columnModel : columns) {
				String qFieldType = getQueryModelFieldType(columnModel.getFieldType());
				sb.append("\tpublic " + qFieldType + " get" + toFirstCharUpCase((String) columnModel.getFieldName())
						+ "() {\r\n" + "\t\treturn " + columnModel.getFieldName() + ";\r\n" + "\t}\r\n" + "\r\n"
						+ "\tpublic void set" + toFirstCharUpCase((String) columnModel.getFieldName()) + "("
						+ qFieldType + " " + columnModel.getFieldName() + ") {\r\n" + "\t\tthis."
						+ columnModel.getFieldName() + " = " + columnModel.getFieldName() + ";\r\n" + "\t}\r\n\r\n");
			}
			sb.append("}\r\n");
		} catch (Exception e) {
	 ...
	 * 生成Dao
	 */
	private static String genDao(String author, String packagePath, String beanName, String queryModelName,
			String extendsBasePackage) {
		StringBuffer sb = new StringBuffer();
		if (StringUtils.isNotEmpty(packagePath)) {
			sb.append("package " + packagePath + ";\n\n");
		}
	 ...
	 * 生成Service
	 */
	private static String genService(String author, String packagePath, String beanName, String queryModelName,
			String extendsBasePackage) {
		StringBuffer sb = new StringBuffer();
		if (StringUtils.isNotEmpty(packagePath)) {
			sb.append("package " + packagePath + ";\n\n");
	 ...
	 * 生成controller
	 */
	private static String genController(String author, String packagePath, String beanName, String queryModelName,
			String extendsBasePackage) {
		StringBuffer sb = new StringBuffer();
		if (StringUtils.isNotEmpty(packagePath)) {
			sb.append("package " + packagePath + ";\n\n");
		...
		sb.append("public class " + beanName + "Controller extends GenericController<" + beanName + ", "
				+ queryModelName + "> {\r\n");
		sb.append("\t@Inject\n");
	 ...
	 * @since properties keys include 'db.driver'、'db.username'、'db.password' and
	 *        'db.url'
	 */
	public static void genFiles(String author, String tableName, String extendsBasePackage, String basePackage,
			String mybatisBasePath, String beanName, String queryModelName, String properitesUri) throws IOException {
		...
		if (StringUtils.isNotEmpty(queryModelName)) {
			...
			fos.write(genQueryModelFromTableStructure(author, table, queryModelName, extendsBasePackage,
					basePackage + ".entity").getBytes());
			fos.close();
		}
		// 生成mybatis配置文件
		...
		fos.write(
				MyBatisUtil
						.genMapperConfig(table, basePackage + ".dao." + beanName + "Dao",
								basePackage + ".entity." + beanName, basePackage + ".entity." + queryModelName)
						.getBytes());
		fos.close();
		// 生成Dao
		...
		fos.write(
				genService(author, basePackage + ".service", beanName, queryModelName, extendsBasePackage).getBytes());
		fos.close();
		// 生成controller
	...
	public static void main(String[] args) throws IOException {
		genFiles("chenaonan", "wmp_users", "com.wmp.common.base", "com.wmp.sys", "/resources/mybatis/mapper", "User",
				"QueryUser", "application-dev.properties");
	}
